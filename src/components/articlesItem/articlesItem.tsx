    ...
    return (
        <li className={clasess.articleItem}>
            <div className={clasess.slugInfo}>
                ...
                {!posts.favorited ? <div onClick={() => 
                dispatch(LikeCurrentArticle(posts.slug,token)
                )}
                 className={clasess.slugLikes}><img src={imgUnLike} className={clasess.imgLikes}/>
                 <div className={clasess['slugLikes-count']}>{posts.favoritesCount}</div></div> 
                 
                 : 
                 
                 <div onClick={() => 
                 dispatch(UnLikeCurrentArticle(posts.slug,token))} 
                 className={clasess.slugLikes}><img src={imgLike} className={clasess.imgLikes}/>
                 <div className={clasess['slugLikes-count']}>{posts.favoritesCount}</div></div>}
                </div>
